...
class PaymentAcquirer(models.Model):
    ...
    provider = fields.Selection(
        selection_add=[('lahza', "Lahza")], ondelete={'lahza': 'cascade'})
    # provider = fields.Selection(selection_add=[('flutterwave', 'Flutterwave')], ondelete={'flutterwave': 'set default'})
    lahza_public_key = fields.Char(required_if_provider='lahza', groups='base.group_user')
    lahza_secret_key = fields.Char(required_if_provider='lahza', groups='base.group_user')
    # rave_secret_hash = fields.Char(required_if_provider='rave', groups='base.group_user', string="Flutterwave Secret Hash")
    environment = fields.Char(required_if_provider='lahza', groups='base.group_user')

    @api.model
    def _get_lahza_api_url(self):
        self.ensure_one()
        """ LahzaURLs"""
        return 'https://api.lahza.co'

    def _lahza_make_request(self, endpoint, payload=None, method='POST', offline=False):
        """ Make a request to Lahza API at the specified endpoint.
        ...
        url = url_join('https://api.lahza.co', endpoint)
        headers = {
            'Authorization': f'Bearer {self.lahza_secret_key}',
                ...
                try:
                    ...
                    raise ValidationError(
                        "Lahza: " + _(
                            ...
                            "Lahza gave us the following info about the problem:\n'%s'", error_msg
                        )
                    )
        except requests.exceptions.RequestException as err:
            _logger.exception("unable to reach endpoint at %s", url)
            _logger.exception("unable to reach endpoint error: %s", err)
            raise ValidationError("Lahza: " + _("Could not establish the connection to the API."))
        _logger.info("Response json: %s", response.json())
        return response.json()

    def _lahza_get_request(self, endpoint, method='GET', offline=False):
        self.ensure_one()
        ...
        try:
            ...
            _logger.exception("Unable to communicate with Lahza: %s", url)
            raise ValidationError("Lahza: " + _("Could not establish the connection to the API."))
        return response.json()
    ...
    def _get_default_payment_method_id(self):
        ...
        return self.env.ref('payment_lahza_ecommerce.payment_method_lahza').id
