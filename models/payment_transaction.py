...
from odoo.addons.payment_lahza_ecommerce.controllers.controllers import LahzaController
...
class PaymentTransaction(models.Model):
    ...
    def _get_specific_processing_values(self, processing_values):
        """ Override of payment to return Lahza-specific processing values.
        ...
        if self.provider != 'lahza' or self.operation == 'online_token':
            return res
        # self.acquirer_reference = self.reference
        payload = self._lahza_prepare_payment_request_payload()
        payload.update({'pub_key': self.acquirer_id.lahza_public_key,})
        return payload
        

    def _get_specific_rendering_values(self, processing_values):
        """ Override of payment to return Lahza-specific rendering values.
        Note: self.ensure_one() from `_get_processing_values`
        ...
        # payment_data = self.acquirer_id._lahza_make_request('/transaction/initialize', payload=payloads)
        ...
        #     'pub_key': self.acquirer_id.lahza_public_key,
        ...
        #     raise ValidationError("Lahza " + _("Unable to generate Payment Link. Try again"))
        # return {'api_url': payment_data.get('data').get('authorization_url')}
        return payload


    def _lahza_prepare_payment_request_payload(self):
        """ Create the payload for the payment request based on the transaction values.
        :return: The request payload
        :rtype: dict
        """
        base_url = self.acquirer_id.get_base_url()
        return_url = LahzaController._checkout_return_url
    ...
    def _get_tx_from_feedback_data(self, provider, data):
        """ Override of payment to find the transaction based on Lahza data.
        ...
        from_lahza_status = data.get('status')

        if not reference:
            raise ValidationError("Lahza: " + _("Received data with missing merchant reference"))

        tx = self.search([('reference', '=', reference ), ('provider', '=', 'lahza')])
        if not tx:
            raise ValidationError(
                "Lahza: " + _("No transaction found matching reference %s.", reference )
            )
        return tx

    def _process_feedback_data(self, data):
        """ Override of payment to process the transaction based on Lahza data.
        ...
        payment_data = self.acquirer_id._lahza_get_request(
        ...
        _logger.info("lahza verify endpoint: %s", payment_status)

        if payment_status == 'success' and (self.amount * 100) == amount and self.currency_id.name != currency:
            self._set_done()
        elif payment_status == 'failed':
            self._set_canceled("Lahza: " + _("Canceled payment with status: %s", payment_status))
        # elif payment_status == 'successful' and (self.amount * 100) != amount:
            ...
            self._set_error(
                "Lahza: " + _("Received data with invalid payment status: %s", payment_status)
            )
